...
public class CoderUtils {
     ...
     * @return
     */
    public static byte[] ucTransfer(byte[] buffer, int Len, boolean Xmit)
    {
        int TempLen;
        ...
        if (Xmit)
        {
            ...
            for (int I = 0; I <= TempLen - 1; I++)
            {
                if ((buffer[I] == DriverConst.Const_FrameHead) || (buffer[I] == DriverConst.Const_FrameTail) || (buffer[I] == DriverConst.Const_FrameTrans))
                {
                    ...
                    Len += 1;
                    int myTrans = buffer[I] & 0xFF;
                    TempBuf.add((byte)(myTrans >> 4)); //buffer[I]
                }
                else
                {
        ...
        else
        {
            ...
            while (J <= (TempLen - 1))
            {
                ...
                {
                    TempBuf.add((byte)((buffer[J + 1] << 4) + buffer[J + 1]));
                    J += 1;
                }
                else
                {
     ...
     * @return
     */
    public static int crc16Table(byte[] bytes) {
        int[] table = {
